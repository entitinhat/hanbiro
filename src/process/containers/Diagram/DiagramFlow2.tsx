...
import ReactFlow, { Background, ReactFlowProvider, FitViewOptions } from 'reactflow';
import {
  NodeAction,
  NodeStatus,
  NodeChecklist,
...
import { RFState, useDiagramStore } from '../../hooks/useDiagramStore';
...
const DiagramFlow = () => {
  ...
  const { reactFlowWrapper, setReactFlowInstance, useStore, nodeHidden, setNodeHidden } = useDiagramStore(
    processId!!,
    diagrams as DiagramData
  );

  const { nodes, edges, onNodesChange, onEdgesChange, onConnect, onNodeDragStart, onNodeDragStop, onEdgeUpdate, onDragOver } = useStore(selector, shallow);
