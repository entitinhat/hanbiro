...
import { KEY_NAME_BUSINESS_CREATED_AT, KEY_NAME_BUSINESS_MODE } from '../../../config/keyNames';

interface BodyProps {
  category: string;
  fields: any[];
  isSplitMode: boolean;
  itemsList: BusinessProcess[];
  paging: Paging;
}

const Body = (props: BodyProps) => {
  console.log('ProcessPage > Body');
  ...
  const EditFn = useCallback((data: BusinessProcess) => {
    console.log('editfn', data);
    setProcessOpen({ open: true, data: data });
  }, []);
  ...
  const tableFields = useMemo(() => {
    ...
    fields.forEach((_ele: any) => {
      if (processOpen.open && _.includes([KEY_NAME_BUSINESS_CREATED_AT, KEY_NAME_BUSINESS_MODE], _ele.keyName)) return;
      newFields.push({
        ..._ele,
        enableSorting: true,
