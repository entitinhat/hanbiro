...
const intlDTCache = {} as Record<string, any>;
...
export const ganttDateRange = (tasks: Task[], viewMode: ViewMode, preStepsCount: number) => {
  let newStartDate: Date = tasks.length > 0 ? tasks[0].start : dayjs().toDate();
  let newEndDate: Date = tasks.length > 0 ? tasks[0].start : dayjs().toDate();
