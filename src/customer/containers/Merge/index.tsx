...
const MERGE_OPTIONS = [
  { value: CUSTOMER_CATEGORY_ACCOUNT, title: 'Account', label: 'Merge Account', icon: 'm_customer_account', iconType: 'icon' },
  { value: CUSTOMER_CATEGORY_CONTACT, title: 'Contact', label: 'Merge Contact', icon: 'contacts', iconType: 'icon' }
];
...
const CustomerMergeModal = (props: CustomerMergeModalProps) => {
  const {
    isOpen,
    onClose,
  ...
  const [activeCate, setActiveCate] = useState<any>(category === CUSTOMER_CATEGORY_ALL ? CUSTOMER_CATEGORY_ACCOUNT : category); //
  const [criteriaOptions, setCriteriaOptions] = useState<any>([]);
  const [formStep, setFormStep] = useState(0);

  //init creiteria option
  useEffect(() => {
    if (activeCate === CUSTOMER_CATEGORY_ACCOUNT) {
      let newOptions = [
    ...
    if (activeCate === CUSTOMER_CATEGORY_CONTACT) {
      let newOptions = [
  ...
  useEffect(() => {
    if (selectedDup) {
      ...
      let query = `category=${CUSTOMER_CATEGORY_ENUM[activeCate]}`;
      switch (searchCriteria.value) {
        case 'CRITERIA_NAME':
          query += ` originName=\"${selectedDup.name}\"`;
          break;
        case 'CRITERIA_EMAIL':
          query += ` ${keyNames.KEY_NAME_CUSTOMER_EMAIL}=\"${selectedDup.name}\"`;
          break;
        case 'CRITERIA_PHONE':
          query += ` ${keyNames.KEY_NAME_CUSTOMER_PHONES}=\"${selectedDup.name}\"`;
          break;
        case 'CRITERIA_MOBILE':
          query += ` ${keyNames.KEY_NAME_CUSTOMER_MOBILE}=\"${selectedDup.name}\"`;
          break;
      }
  ...
  const handleSearch = () => {
    if (searchCriteria) {
      ...
      if (customer) {
        const primaryEmail = customer?.[keyNames.KEY_NAME_CUSTOMER_EMAIL]?.find((_ele: any) => _ele.label.label === 'LABEL_PRIMARY');
        const primaryPhone = customer?.[keyNames.KEY_NAME_CUSTOMER_PHONES]?.find((_ele: any) => _ele.label.label === 'LABEL_PRIMARY');
        const primaryMobile = customer?.[keyNames.KEY_NAME_CUSTOMER_MOBILE]?.find((_ele: any) => _ele.label.label === 'LABEL_PRIMARY');
        ...
        switch (searchCriteria.value) {
          case 'CRITERIA_NAME':
            ...
            break;
          case 'CRITERIA_EMAIL':
            query += ` ${keyNames.KEY_NAME_CUSTOMER_EMAIL}=\"${primaryEmail?.email || ''}\"`;
            break;
          case 'CRITERIA_PHONE':
            query += ` ${keyNames.KEY_NAME_CUSTOMER_PHONES}=\"${primaryPhone?.phoneNumber || ''}\"`;
            break;
          case 'CRITERIA_MOBILE':
            query += ` ${keyNames.KEY_NAME_CUSTOMER_MOBILE}=\"${primaryMobile?.mobile || ''}\"`;
            break;
        }
      }
      newFilter.filter.query = query;
  ...
  const renderMasterStep = () => {
    return (
      <Stack spacing={1.25} sx={{ p: 2 }}>
        <InputLabel>Select Master {activeCate === CUSTOMER_CATEGORY_ACCOUNT ? 'Account' : 'Contact'}</InputLabel>
  ...
  return (
    <MiModal
      title={category === CUSTOMER_CATEGORY_ACCOUNT ? 'Merge Account' : 'Merge Contact'}
      isOpen={isOpen}
      size="sm"
      fullScreen={false}
      onClose={onClose}
