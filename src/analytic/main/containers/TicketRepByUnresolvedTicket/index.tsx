...
const TicketRepByUnresolvedTicket = (props: ChartComponentProps) => {
  ...
  const chartData = results?.reduce(
    (f: any, v: any) => {
      f['l'].push(v?.user?.name ?? '--No User Name--');
      f['s'].push(v?.value ?? 0);
      return f;
    },
    { l: [], s: [] },
...
export default TicketRepByUnresolvedTicket;
