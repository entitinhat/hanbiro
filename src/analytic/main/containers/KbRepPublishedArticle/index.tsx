...
const KbRepPublishedArticle = (props: ChartComponentProps) => {
  const {data} = useChartQuery(props, () => {
    return useGetKbRepCounting(props?.filters ?? {}, '', {forWhichChart: 'KbRepPublishedArticle'});
  });
  ...
  if (!!results && !!results?.length) {
    chartData = results?.reduce((f: any, v: any) => {
      ...
      f['s'].push(v?.counting?.published ?? 0);
      return f;
    }, chartData);
  }
...
export default KbRepPublishedArticle;