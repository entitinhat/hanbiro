...
const TicketGroupPerformanceByAssignedTicket = (props: ChartComponentProps) => {
  const {filters, me, ...restProps} = props;
  ...
  const chartData = results?.reduce(
    (f: any, v: any) => {
      ...
      f['s'].push(v?.counting?.assigned ?? 0);
      return f;
    },
    { l: [], s: [] },
  ) ?? { l: [], s: [] };
...
export default TicketGroupPerformanceByAssignedTicket;
