...
import CampaignCard from '@base/components/@hanbiro/PreviewCards/CampaignCard';
      ...
      <MainCard content border={false}>
        ...
        <Box ref={printRef} className="scroll-box" sx={{}}>
          <Grid container sx={{ width: '984px' }}>
            {layoutData.map((item: Campaign, index: number) => {
              return (
                <Grid key={index} item xs={12 / selectedOption} sx={{ p: '10px', pageBreakInside: 'avoid' }}>
                  <CampaignCard data={item} columns={selectedOption} />
                </Grid>
              );
            })}
          </Grid>
