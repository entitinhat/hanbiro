...
import { Box, Button, Divider, Drawer, FormGroup, Grid, Stack, Typography, useMediaQuery, useTheme } from '@mui/material';
...
const WriteCommentForm: React.FC<WriteReplyFormProps> = (props) => {
  const { knowledgeId, knowledgeName, onReload, onClose, isOpen, title, menuSource } = props;
  ...
  const {
    ...
    defaultValues: {
      [keyNames.KEY_KNOWLEDGE_BASE_COMMENT_CONTENT]: '',
      [keyNames.KEY_KNOWLEDGE_BASE_COMMENT_ATTACHEDFILES]: []
    },
    criteriaMode: 'firstError',
    mode: 'onChange'
  });
  //Mutation
  const { mutationAdd, mUpload } = useKnowledgeCommentMutation();
  //responsive
  ...
  const onUploadCompleted = (files: S3UploadedFile[]) => {
    ...
    const params = {
      comment: {
        content: submitData[keyNames.KEY_KNOWLEDGE_BASE_COMMENT_CONTENT],
        parent: null,
        kb: { id: knowledgeId, name: knowledgeName },
        attachedFiles: files
      }
    };
      ...
      <>
        <Box sx={{ p: 1 }}>
          <Grid container justifyContent="space-between" alignItems="center" sx={{ px: 1 }}>
            <Grid item>
              <SpanLang sx={{ color: theme.palette.text.primary, fontSize: '18px', fontWeight: '700' }} keyLang={title} />
            </Grid>
          </Grid>
        </Box>
        <Divider />
        ...
        <Grid sx={{ padding: '15px', height: 'calc(100vh - 150px)' }} className="scroll-box">
          <Grid sx={{ marginBottom: '1rem' }}>
            ...
            <Grid sx={{ flex: '1 1 auto' }}>
              <Controller
                name={keyNames.KEY_KNOWLEDGE_BASE_COMMENT_CONTENT}
                control={control}
                rules={{ required: true }}
          ...
          <Grid sx={{ marginBottom: '1rem' }}>
            ...
            <Box sx={{ flex: '1 1 auto' }}>
              <Controller
                name={keyNames.KEY_KNOWLEDGE_BASE_COMMENT_ATTACHEDFILES}
                control={control}
