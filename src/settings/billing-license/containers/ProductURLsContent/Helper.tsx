...
export const columnRenderRemap = (menu: string, menuSourceId?: string, valueType?: any) => ({
  ...
  url(col: string, data: any) {
    console.log('url', col, data);
    let url = data[col] ? data[col] : '';
    return <RouteName name={url} url={url} />;
  },
  ...
  id(col: string, data: any) {
    ...
    return <UpdateURL id={reps} sx={{ width: 1 }} menuSourceId={menuSourceId} value={content?.content || ''} />;
  },
...
export const productURLsFields = [
  //-----------------------------------------------------Content-----------------------------------
  {
    name: 'Sites and products',
    isDefault: true,
    sortable: false,
    dataType: 'text',
    order: 11,
    title: 'Sites and Products',
    hidden: false,
    id: 'content',
    keyName: 'content', //Table get keyName to map data
    languageKey: 'Sites and Products',
    showInList: true,
    orderInList: 11,
    showInView: true,
    defaultViewInList: true,
