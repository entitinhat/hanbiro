...
import { TEMPLATE_STAGE_OPTIONS, TEMPLATE_TYPE_OPTIONS_ENUM } from '../constants';
...
export const groupByOptionsKB: LabelValue[] = [
...
export const filterByOptionsKB = [
  {
    ...
    setValue: (value: number) => {
      return TEMPLATE_TYPE_OPTIONS_ENUM?.find((v) => v.value === value);
    }
  },
  {
  ...
  },
  {
    ...
    getValue: (value: User[]) => {
      return value?.length > 0 ? value?.map((v) => v?.id).join(',') : '';
    },
    setValue: (value: string) => {
      return value ? value.split(',') : [];
    }
