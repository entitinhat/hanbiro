...
import { usGetSurveyList } from '../../hooks/useGetSurveyList';
...
const SurveyAutocomplete = (props: EditProps) => {
  ...
  //params: sample
  const { data: surveyData, isLoading } = usGetSurveyList(filter);

  //convert to select options
  useEffect(() => {
    if (surveyData?.data) {
      let newOptions = surveyData?.data?.map((_item: any) => {
        return {
          ..._item,
          label: _item.name,
          value: _item.id
  ...
  return (
    <Autocomplete
      ...
      options={options || []}
      renderInput={(params) => (
        <TextField {...params} placeholder={t(placeholder || 'ncrm_generalsetting_survey_autocomplete_placeholder') as string} />
      )} // translate language
