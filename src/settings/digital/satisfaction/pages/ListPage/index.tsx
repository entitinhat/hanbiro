...
//menu
import ListBottomToolbar from '@settings/digital/satisfaction/containers/ListBottomToolbar';
import { useSatisfactionSurveys } from '@settings/digital/satisfaction/hooks/useSatisfactionSurveys';
...
const ListPage = ({ isSplitMode = false }: ListPageProps) => {
  ...
  let layoutCategory = 'satisfaction'; //router category
  ...
  //get page setting
  const pageDataKey = `setting_${layoutCategory}_survey`;
  ...
  //get list data
  const { data: listData, isFetching: isDataFetching, refetch } = useSatisfactionSurveys(viewingFields);
