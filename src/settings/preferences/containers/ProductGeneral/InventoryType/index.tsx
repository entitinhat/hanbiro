...
const InventoryType = () => {
  ...
  const keyName = 'default_inventory_type';
  ...
  return (
    <MainCard content title={<SpanLang keyLang={'ncrm_setting_product_default_inventory_type'} />}>
      <FormControl>
        <RadioGroup
          row
          name="measure-weight"
          value={value || PRODUCT_ITEM_INVENTORY_TYPE_OPTIONS[0].value}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange(e.target.value as string)}
        >
          {PRODUCT_ITEM_INVENTORY_TYPE_OPTIONS?.map((item: any, index: number) => (
