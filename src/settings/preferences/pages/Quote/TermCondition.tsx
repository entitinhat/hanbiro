...
import { KEY_QUOTE_TERM_CONDITION } from '../Quote';

export interface QuoteTermCondition {
  id: string;
  title: string;
  description: string;
  content: string;
...
interface TermConditionProps {
  defaultValue: QuoteTermCondition[];
}

const TermCondition = (props: TermConditionProps) => {
  const { defaultValue } = props;
  //state
  const [items, setItems] = useState<QuoteTermCondition[]>(defaultValue);
  //hook
  ...
  //save item
  const handleSave = (newData: QuoteTermCondition[]) => {
    const params: any = {
      menu: MENU_QUOTE,
      key: KEY_QUOTE_TERM_CONDITION,
  ...
  const columns = React.useMemo<ColumnDef<any>[]>(
    () => [
      ...
      {
        accessorKey: 'content',
        header: () => <span>{t('ncrm_generalsetting_preferences_quote_main_content')}</span>
      },
      {
  ...
  //add new row
  const handleAddRow = () => {
    const defaultRow: QuoteTermCondition = {
      id: generateUUID(),
      title: '',
      description: '',
      content: '',
  ...
  //table data change
  const handleDataChange = (newData: QuoteTermCondition[]) => {
    setItems(newData);
    //save
    handleSave(newData);
  };

  //console.log('Terms items', items);
  return (
    <MainCard title={t('ncrm_generalsetting_preferences_quote_terms_and_conditions')}>
      ...
      {/* <Button color="primary" startIcon={<AddOutlined />} sx={{ mt: 1 }} onClick={handleAddRow}>
