...
import ListGridCard from '@opportunity/containers/ListGridCard';
import { getMapColumns, isDeleteList } from '@opportunity/pages/ListPage/Helper';
import {
  DISABLED_SORT_FIELDS,
  OPPORTUNITY_GROUP_BY_COMPETITOR,
  OPPORTUNITY_GROUP_BY_CUSTOMER,
  OPPORTUNITY_GROUP_BY_DELETED,
  OPPORTUNITY_GROUP_BY_MY_GROUP,
  OPPORTUNITY_GROUP_BY_PROCESS,
  OPPORTUNITY_GROUP_BY_PRODUCT
} from '@opportunity/config/list-field';
import { useOpportunityRestore, useOpportunityEmpty, useOpportunityDeleteRecovery } from '@opportunity/hooks/useOpportunityDelete';
...
const Body = (props: BodyProps) => {
  ...
  //delete action
  const mRestore = useOpportunityRestore({ onCancel, onReload });
  const mEmpty = useOpportunityEmpty({ onCancel, onReload });
  const mDeleteRecovery = useOpportunityDeleteRecovery({ onCancel, onReload });
  //groupBy UI
  ...
  // group by type 1
  if (
    groupBy == OPPORTUNITY_GROUP_BY_MY_GROUP ||
    groupBy == OPPORTUNITY_GROUP_BY_PRODUCT ||
    groupBy == OPPORTUNITY_GROUP_BY_CUSTOMER ||
    groupBy == OPPORTUNITY_GROUP_BY_COMPETITOR ||
    groupBy == OPPORTUNITY_GROUP_BY_PROCESS
    //groupBy == OPPORTUNITY_GROUP_BY_DELETED
  ) {
    selectedGroupType = GroupType.ROWSPAN;
    switch (groupBy) {
      case OPPORTUNITY_GROUP_BY_MY_GROUP:
        groupKeyName = keyNames.KEY_NAME_OPPORTUNITY_CREATED_BY; //single
        break;
      case OPPORTUNITY_GROUP_BY_PRODUCT:
        groupKeyName = keyNames.KEY_NAME_OPPORTUNITY_PRODUCT; //multiple
        break;
      case OPPORTUNITY_GROUP_BY_CUSTOMER:
        groupKeyName = keyNames.KEY_NAME_OPPORTUNITY_CUSTOMER; //single
        break;
      case OPPORTUNITY_GROUP_BY_COMPETITOR:
        groupKeyName = keyNames.KEY_NAME_OPPORTUNITY_COMPETITOR; //multiple
        break;
      case OPPORTUNITY_GROUP_BY_PROCESS:
        groupKeyName = keyNames.KEY_NAME_OPPORTUNITY_PROCESS; //single
        break;
      // default: //single
      //   groupKeyName = keyNames.KEY_NAME_OPPORTUNITY_CREATED_BY;
      //   break;
    }
  }
  // group by type 2
  ...
  const bottomHeaderDeletedProps = {
    ...
    leftFilter: [
      {
        value: keyNames.KEY_NAME_OPPORTUNITY_DELETED_BY,
  ...
  }, [itemsList, paging, tableColumns, checkedIds, groupBy]);
  ...
  }, [itemsList, paging, isSplitMode, viewingFields, checkedIds]);
