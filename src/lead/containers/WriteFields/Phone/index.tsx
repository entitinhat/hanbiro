...
const PhoneInput = (props: PhoneInputProps) => {
  ...
  useEffect(() => {
    if (countryPhones.length > 0) {
      ...
      if (defaultPhone) {
        setPhones(phones.map((_ele: any) => ({ ..._ele, country: _ele.country === '' ? defaultPhone.phoneCode : _ele.country })));
      }
    }
  }, [countryPhones]);
            ...
            {isMultiple && (
              <Stack direction="row">
                ...
                {isMultiple && idx === phones.length - 1 && (
                  <Button color="primary" size="small" startIcon={<AddCircleOutline />} onClick={handeAdd}>
