...
import ListGridCard from '@blocklist/containers/ListGridCard'; //custom
...
import { defaultColumnOrder } from '@blocklist/config/list-field';
...
const PageBody = (props: BodyProps) => {
  ...
  const tableFields = useMemo(() => {
    ...
    if (fields?.length > 0 && category !== 'block_list' && category !== 'marketing_list') {
      viewingFields = getViewingFields(fields, settingColumns);
    } else if (category === 'block_list' || category === 'marketing_list') {
  ...
  const getConfigRowSpanned = (data: any, spannedKeyName: string, spannedByAttr: string) => {
  ...
  const getAccordionData = (keyName: string, keyNameGroupBy: string) => {
    const itemListGroupByKeyName: any = [];
    itemsList?.forEach((item: any, index: number) => {
      const fIndex = itemListGroupByKeyName.findIndex((v: any) => v[keyName]?.[keyNameGroupBy] == item[keyName]?.[keyNameGroupBy]);
      if (fIndex > -1) {
  ...
  console.log('getConfigRowSpanned: ', getConfigRowSpanned(itemsList, keyNames.KEY_NAME_CUSTOMER_CREATED_BY, 'id'));
  console.log('isRowSpanned: ', isRowSpanned);
  console.log('getAccordionData: ', getAccordionData(keyNames.KEY_NAME_CUSTOMER_CREATED_BY, 'id'));
  // Table
  ...
  if (isRowSpanned) {
    if (groupBy === 'myGroupBlockList_1') {
      tableItemList = getConfigRowSpanned(itemsList, keyNames.KEY_NAME_CUSTOMER_CREATED_BY, 'id') || [];
    }
    if (groupBy === 'blockListperCampaign_1') {
      tableItemList = getConfigRowSpanned(itemsList, keyNames.KEY_NAME_CUSTOMER_CAMPAIGN, 'id') || [];
    }
    if (groupBy === 'blockListperCustomer_1') {
      tableItemList = getConfigRowSpanned(itemsList, keyNames.KEY_NAME_CUSTOMER_CUSTOMER, 'id') || [];
    }
  } else {
  ...
  const TableGroupByKeyName = (keyName: string) => {
    return getAccordionData(keyName, 'id').map((item: any, i: number) => {
  ...
  const GridMemo = useMemo(() => {
    ...
    //   columnRenderRemap: getMapColumns(category), //isSplitMode ? getMapColumnsInSplitMode(category) : getMapColumns(category),
    //   hideColumns,
    //   isSmall: isSplitMode,
    //   children: () => <></>
    // };

    //return <ListGrid {...listGridProps}>{(props) => <ListGridCard {...props} category={category} />}</ListGrid>;
    ...
    switch (listType) {
      ...
      default:
        if (isGroupByAccordion) {
          if (groupBy === 'myGroupBlockList_2') {
            return (
              <>
                {TableGroupByKeyName(keyNames.KEY_NAME_CUSTOMER_CREATED_BY)}
                {pagingProps && (
                  <Box sx={{ mt: 2, px: 2 }}>
                    <ListPagination
          ...
          if (groupBy === 'blockListperCampaign_2') {
            return (
              <>
                {TableGroupByKeyName(keyNames.KEY_NAME_CUSTOMER_CAMPAIGN)}
                {pagingProps && (
                  <Box sx={{ mt: 2, px: 2 }}>
                    <ListPagination
                      gotoPage={(page: number) => handlePagingChange && handlePagingChange(page, pagingProps.pageSize)}
          ...
          if (groupBy === 'blockListperCustomer_2') {
            return (
              <>
                {TableGroupByKeyName(keyNames.KEY_NAME_CUSTOMER_CUSTOMER)}
                {pagingProps && (
                  <Box sx={{ mt: 2, px: 2 }}>
                    <ListPagination
                      gotoPage={(page: number) => handlePagingChange && handlePagingChange(page, pagingProps.pageSize)}
