...
const ItemTypeSelect = (props: ItemTypeProps) => {
  ...
  useEffect(() => {
    if (value) {
      if (_.isString(value)) {
        const newValue: string = PRODUCT_ITEM_TYPE_OPTIONS?.find((v: LabelValue) => v.value === (value as string))?.value as string;
        setSelected(newValue ?? '');
      } else {
        const newValue: string = PRODUCT_ITEM_TYPE_OPTIONS?.[(value as number) - 1]?.value as string;
        setSelected(newValue ?? '');
      }
    } else {
      setSelected('');
  ...
  const handleChange = (event: SelectChangeEvent) => {
    ...
    const newSelected: KeyValue = {
      id: newValue,
      name: PRODUCT_ITEM_TYPE_OPTIONS?.find((v: LabelValue) => v.value === newValue)?.label
    };
    setSelected(newValue);
  ...
  return (
    <FormControl fullWidth>
      ...
      <Select labelId="item-type-simple-select-label" id="item-type-simple-select" value={selected} onChange={handleChange}>
        {PRODUCT_ITEM_TYPE_OPTIONS?.map((option: LabelValue, index: number) => (
