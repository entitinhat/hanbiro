...
import { CHANGE_PRODUCT_ASSIGN_REP, CREATE_PRODUCT_ASSIGN_REP, DELETE_PRODUCT_ASSIGN_REP } from '../services/graphql';
...
export const useAssignRepMutation = (isViewPage: boolean, filterKeys: any[]) => {
  ...
  let firstQuery = queryKeys.listProduct;
  if (isViewPage) {
  ...
  const syncAddMutate = async (variables: any) => {
    ...
    console.log('syncAddMutate variables.assignTo', variables.assignTo);
    const optimistic = variables.assignTo.map((item:AssignToName)=>{
      return item.user
    })
    ...
    queryClient.setQueryData(rQueryKeys, (old: any) => {
      ...
      if (isViewPage) {
        ...
        const nData = data.map((_item: Product) => {
  ...
  const mAssignRep = useMutationPost<BaseMutationBulkKeysResponse>(CREATE_PRODUCT_ASSIGN_REP, queryKeys.createAssignRep, {
  ...
  const mUpdateRep = useMutationPost<BaseMutationBulkKeysResponse>(CHANGE_PRODUCT_ASSIGN_REP, queryKeys.updateProductBatch, {
    onMutate: syncUpdateMutate,
    onSettled: settledMutate,
    onError: errorMutate,
    onSuccess: () => enqueueSuccessBar('Update assign rep successfully!')
  });

  const mDeleteRep = useMutationPost<BaseMutationBulkKeysResponse>(DELETE_PRODUCT_ASSIGN_REP, queryKeys.deleteAssignRep, {
  ...
  return { mAssignRep, mDeleteRep, mUpdateRep };
};
