export default (editor: any, options: any) => {
  const remoteIcons = 'https://github.com/nhn/tui.image-editor/tree/master/apps/image-editor/src/svg/';
  const opts = {
    ...{
  ...
  const getConstructor = () => opts.constructor || ((window as any).tui && (window as any).tui.ImageEditor);
  ...
  const typeImage = domc.getType('n-image')?.model;
  domc.addType('n-image', {
    model: {
      initToolbar(this: any) {
        ...
        const tbExists = tb.some((item: any) => item.command === commandId);
  ...
  editor.Commands.add(commandId, {
    run(ed: any, s: any, options: any) {
      ...
      ed.Modal.open({ title, content, attributes: { class: 'gts-modal-container' } })
        .getModel()
    ...
    createContent() {
      ...
      content.innerHTML = `
        <div></div>
        <button type="button" class="tui-image-editor__apply-btn" style="
          position: absolute;
          top: 0; right: 0;
          margin: 10px;
    ...
    uploadImage(imageEditor: any, target: any, am: any) {
      const dataURL = imageEditor.toDataURL();
      if (upload) {
