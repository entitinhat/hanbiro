...
export default class Undo {
  ...
  maxLength: any;
  onUpdate: any;
  config: { debounceTimer: any; shortcuts: any };
  initialItem: any;
  stack: any;
  position: number;
   ...
   */
  constructor({ editor, config = {}, onUpdate, maxLength }: any) {
  ...
  save(state: any) {
    ...
    const caretIndex =
      state[indexInState] && (state[indexInState].type === 'paragraph' || state[indexInState].type === 'header')
   ...
   */
  blockWasSkipped(index: number, compIndex: number, state: any, compState: any) {
  ...
  undo() {
    if (this.canUndo() && this.position >= 0) {
      ...
      if (this.blockWasDeleted(state, nextState)) {
        ...
        this.blocks.render({ blocks: state }).then(() => this.setCaretIndex(index, caretIndex));
      } else if (this.blockWasDropped(state, nextState)) {
        this.blocks.render({ blocks: state }).then(() => this.caret.setToBlock(index, 'end'));
   ...
   * @param {Number} index is the block index
   */
  insertBlock(state: any, index: number) {
   ...
   * @param {Array} state is the current state according to this.position.
   */
  insertSkippedBlocks(prevStateLength: number, state: any) {
  ...
  parseKeys(keys: any) {
    ...
    const parsedKeys = keys.slice(0, -1).map((key: string) => specialKeys[key]);

    const letterKey =
      parsedKeys.includes('shiftKey') && keys.length === 2 ? keys[keys.length - 1].toUpperCase() : keys[keys.length - 1].toLowerCase();
  ...
  setEventListeners() {
    ...
    const twoKeysPressed = (e: any, keys: any) => keys.length === 2 && e[keys[0]] && (e.key === keys[1] || getRawKey(e.code) === keys[1]);
    const threeKeysPressed = (e: any, keys: any) =>
      keys.length === 3 && e[keys[0]] && e[keys[1]] && (e.key === keys[2] || getRawKey(e.code) === keys[2]);

    const pressedKeys = (e: any, keys: any, compKeys: any) => {
