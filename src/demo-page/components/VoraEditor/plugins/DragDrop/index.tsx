...
export default class DragDrop {
   ...
   */
  constructor({ configuration, blocks, toolbar, save }: any, borderStyle?: string) {
   ...
   */
  setElementCursor(element: HTMLElement) {
    if (!element) return;
  ...
  setDragListener() {
    if (!this.readOnly) {
      ...
      settingsBtn.addEventListener('dragstart', () => {
        this.startBlock = this.api.getCurrentBlockIndex();
      });
      settingsBtn.addEventListener('drag', () => {
  ...
  /**
   * Sets dynamically the borders in the blocks when a block is dragged
   ...
   */
  setBorderBlocks(allBlocks: any, blockFocused: HTMLElement) {
    Object.values(allBlocks).forEach((block: any) => {
      ...
      if (block !== blockFocused) {
        ...
        if (Number(index) > this.startBlock) blockContent.style.borderBottom = this.borderStyle;
  ...
  setDropListener() {
    document.addEventListener('drop', (ev: any) => {
      ...
      if (this.holder.contains(target) && this.startBlock !== null) {
        ...
        if (dropTarget) {
          const blockContent = dropTarget.querySelector('.ce-block__content') as HTMLElement;
          blockContent.style.removeProperty('border-top');
          blockContent.style.removeProperty('border-bottom');
  ...
  getTargetPosition(target: HTMLElement): number {
    return Array.from(target.parentNode?.children as any).indexOf(target);
  }
