export default class XyButton extends HTMLElement {
  btn: any;
  ico: any; //HTMLElement | null | undefined;
  ...
  constructor() {
    ...
    shadowRoot.innerHTML = `
      ...
      */
      ...
      <${this.href ? 'a' : 'button'} ${this.htmltype ? 'type="' + this.htmltype + '"' : ''} ${
      this.download && this.href ? 'download="' + this.download + '"' : ''
    } ${this.href ? 'href="' + this.href + '" target="' + this.target + '" rel="' + this.rel + '"' : ''} class="btn" id="btn"></${
      this.href ? 'a' : 'button'
    }>${!this.loading && this.icon && this.icon != 'null' ? '<xy-icon id="icon" name=' + this.icon + '></xy-icon>' : ''}<slot></slot>
      `;
    this.load = null;
  }
  ...
  get htmltype() {
    return this.getAttribute('htmltype') as string;
  }
  ...
  connectedCallback() {
    ...
    this.btn.addEventListener('mousedown', (ev: any) => {
      //ev.preventDefault();
      //ev.stopPropagation();
      if (!this.disabled) {
    ...
    this.btn.addEventListener('keydown', (ev: any) => {
      switch (ev.code) {
        case 13: //Enter
          ev.stopPropagation();
          break;
  ...
  attributeChangedCallback(name: string, oldValue: any, newValue: any) {
    ...
    if (name == 'loading' && this.btn) {
      if (newValue !== null) {
        this.shadowRoot?.prepend(this.load as HTMLElement);
        this.btn.setAttribute('disabled', 'disabled');
      } else {
        this.shadowRoot?.removeChild(this.load as HTMLElement);
        this.btn.removeAttribute('disabled');
      }
    }
