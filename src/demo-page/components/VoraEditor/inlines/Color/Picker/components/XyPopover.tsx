class XyPopcon extends HTMLElement {
  titles: HTMLElement | null | undefined;
  btnClose: HTMLElement | null | undefined;
  btnCancel: HTMLElement | null | undefined;
  btnSubmit: HTMLElement | null | undefined;
  isRemove: boolean;
  ...
  set open(value) {
    if (value === null || value === false) {
      ...
      (this.parentNode as HTMLElement)?.removeAttribute('open');
    } else {
      this.setAttribute('open', '');
      (this.parentNode as HTMLElement)?.setAttribute('open', '');
...
class XyPopover extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (this.trigger === 'contextmenu') {
      this.addEventListener('contextmenu', (ev: any) => {
        ev.preventDefault();
        const path = ev.path || (ev.composedPath && ev.composedPath());
        if (!path.includes(this.popcon)) {
          this.show(ev);
