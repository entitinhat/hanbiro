...
export default class ColorPlugin extends HTMLElement {
  ...
  colorBtn: HTMLElement | null | undefined;
  popover: HTMLElement | null | undefined;
  popcon: HTMLElement | null | undefined;
  colors: HTMLElement | null | undefined;
  nativeclick: boolean;
  pickerInput: any;
  customPicker: HTMLElement | null | undefined;
  $value: any;
  colorPane: any;
  ...
  constructor(options: any) {
    ...
    shadowRoot.innerHTML = `
      ...
      .color-btn {
          ...
          opacity: 0.9;
          padding: 1px 0 1px 0;
          color: transparent; //var(--themeColor, #42b983);
              ...
              <xy-popcon id="popcon">
                  <div class="color-sign" id="colors">
                      ...
                      ${this.colorCollections
                        .map((el: any) => '<button class="color-cube" style="background-color:' + el + '" data-color=' + el + '></button>')
                        .join('')}
                  </div>
              </xy-popcon>
          </xy-popover>
  ...
  connectedCallback() {
    ...
    this.colors?.addEventListener('click', (ev: any) => {
  ...
  closeConverter() {
    ...
    if (conversionOpened) {
      ...
      (converterBtn as HTMLElement)?.click();
    }
  }
  ...
  setupCustomPicker() {
    ...
    const customPicker: any = this.customPicker;
    customPicker.style.backgroundColor = this.customColor;
    this.customPicker?.addEventListener('click', (ev: any) => {
      ...
      const rect: any = this.popcon?.getBoundingClientRect();
  ...
  set value(value) {
    ...
    this.colorBtn?.style.setProperty(
      '--themeColor',
      this.nativeclick ? setDefaultColorCache(value, this.pluginType) : getDefaultColorCache(value, this.pluginType)
    );
    if (this.nativeclick) {
      this.nativeclick = false;
      this.dispatchEvent(
