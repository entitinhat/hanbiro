...
export default class Table {
   ...
   */
  constructor(readOnly: boolean, api: API, data: any, config: ToolConfig) {
  ...
  bindEvents() {
    ...
    this.table.addEventListener(
      'mousemove',
      throttled(150, (event: any) => this.onMouseMoveInTable(event)),
      { passive: true }
    );

    // Controls some of the keyboard buttons inside the table
    this.table.onkeypress = (event: any) => this.onKeyPressListener(event);

    // Tab is executed by default before keypress, so it must be intercepted on keydown
    this.table.addEventListener('keydown', (event: any) => this.onKeyDownListener(event));

    // Determine the position of the cell in focus
    this.table.addEventListener('focusin', (event: any) => this.focusInTableListener(event));
  }
   ...
   * @returns {Element}
   */
  getRowFirstCell(row: HTMLElement) {
   ...
   */
  focusInTableListener(event: any) {
  ...
  unselectColumn() {
    ...
    Array.from(cells).forEach((column: any) => {
  ...
  getHoveredCell(event: any) {
    ...
    if (x >= 0) {
      hoveredColumn =
        this.binSearch(
          ...
          (mid: any) => this.getCell(1, mid),
          ({ fromLeftBorder }: any) => x < (fromLeftBorder as number),
          ({ fromRightBorder }: any) => x > width - fromRightBorder
        ) || 0;
    }
    ...
    if (y >= 0) {
      hoveredRow =
        this.binSearch(
          this.numberOfRows,
          (mid: any) => this.getCell(mid, 1),
          ({ fromTopBorder }: any) => y < (fromTopBorder as number),
          ({ fromBottomBorder }: any) => y > height - fromBottomBorder
        ) || 0;
    }
   ...
   * @returns {number}
   */
  binSearch(numberOfCells: number, getCell: any, beforeTheLeftBorder?: any, afterTheRightBorder?: any) {
    let leftBorder = 0;
